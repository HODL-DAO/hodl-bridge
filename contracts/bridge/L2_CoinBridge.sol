...
/**
 * @title L2_CoinBridge
 * @dev L2_CoinBridge uses the state of L1 Bridge to validate and mint a wrapped token on L2
 *      as an example we wrap DevETH (DTH) onto the CheapETH chain as WDTH
 *      this contract validates that the dTH has been locked into the L1_CoinLock contract
 */
contract L2_CoinBridge {
    Bridge immutable bridge;
    address immutable wrappedToken;
    ...
    constructor(Bridge input_bridge, address token, address dep, uint16 chainId) public {
        bridge = input_bridge;
        wrappedToken = token;
        depositOnL1 = dep;
        chainIdOnL1 = chainId;
    ...
    function redeemDeposit(bytes memory rlpBlockHeader, bytes memory rlpTransaction, address payable inputFrom, bytes memory key, bytes memory proof) public {
        ...
        // mint wrapped tokens
        IERC20(wrappedToken).mint(address(inputFrom), value);
    }
}